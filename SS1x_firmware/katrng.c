                 ...
                 int security_strength)
{
	...
	// memcpy는 <string.h> 파일에 선언되어 있는 함숭디ㅏ.
	// seed_materail 주소에 entropy_input 값을 48개 복사한다.

    memcpy(seed_material, entropy_input, 48);

	// 현재 값에서는 personaliztion_string이 NULL 값이기 때문에
	// 이 부분이 실행되지 않는다.
	...
	*/
	...
	// memcpy는 주소에 있는 값들을 다른 주소에 복사하는 역할을 하는 역할이다.
	// memset은 내가 원하는 주소에 원한느 값으로 세팅할 수 있다.
	// 여기에서는 0x00(0)의 값으로 32길이 or 16길이 만큼 초기화를 해주었다.

    memset(DRBG_ctx.Key, 0x00, 32);
    memset(DRBG_ctx.V, 0x00, 16);


	// seed_material에서는 우리가 위에서 entropy_input에 있는 값들을 복사했다.
	// DRBG_ctx.Key
	// DRBG가 난수 생서기라고 한다. AES를 통해서 난수를 생성하는 것으로 보인다. 
...
// AES256_CTR_DRBG_Update의 목적은
// DBRG 값의 Key와 V를 AES256_ECB를 통해서 만든 48바이트 값으로
// 초기화 시켜주는 역할을 한다. 
void
AES256_CTR_DRBG_Update(unsigned char *provided_data,
                       ...
                       unsigned char *V)
{	
	// 여기 들어올 때도 DRBG_ctx.key와 DRBG_ctx.V는 0으로 초기화 되어있다. 
	// AES256_CTR_DRBG_Update(seed_material, DRBG_ctx.Key, DRBG_ctx.V);
	// provided_data = seed_material이다. 여기에서는 0~47까지 초기화된 값이 들어간다.
	// 이때 seed_material값은 entropy_input 값에서 가져온 것이다.
    unsigned char   temp[48];
    

    for (int i=0; i<3; i++) {
        //increment V
		// 여기에서 V[j]값을 수정한다. V[j]는 1씩만 올리고 바로 반복문을 나간다.
        for (int j=15; j>=0; j--) {
            if ( V[j] == 0xff )
                V[j] = 0x00;
            else {
		...
		// V[j]값을 증가 시키고 나서 수행하는 함수 부분이다.
		// 아래의 부분은 총 3번을 거쳐서 수행을 한다.
		// 첫 번째 호출 시에는
		// Key의 값은 그대로 0이고 V값은 위에서 1이 증가했다.
		// temp은 0~15 / 16~31 / 32~47 와 같은 배열의 값들이 들어간다.
		// 이것을 총 3번 거쳐서 temp에 있는 48바이트 값 모두를 수정한다.
	...
	// AES256_CTR_DRBG_struct 구조체에 있는
	// Key값과 V 값을 위의 AES256_EBC를 통해서 만든 temp를 통해 초기화함
	// temp의 앞의 32바이트가 Key가 된다.
	// temp의 뒤의 16바이트가 V가 된다.
    memcpy(Key, temp, 32);
    memcpy(V, temp+32, 16);
}
...
AES256_ECB(unsigned char *key, unsigned char *ctr, unsigned char *buffer)
{
	// 32바이트 짜리 0으로 초기화된 key가 처음에 들어간다.
	// 16바이트 짜리 0으로 칙화된 V가 처 음에 들어가게 된다.
	// 처음에는 temp+16+0 = temp가 들어간다.
	...
	// buffer에 skey가 들어가게 된다.
	// buffer에는 호출 하기전에 선언했던 temp가 들어가게 된다.
	// temp에는 48바이트가 있지만 16바이트씩 들어간다. 
